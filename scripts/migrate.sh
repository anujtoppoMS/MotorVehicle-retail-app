...
/opt/bin/python /BillingSystem/manage.py collectstatic --noinput
...
if /opt/bin/python /BillingSystem/manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); exit(User.objects.filter(username='$SUPER_USER').exists())"; then
    echo "Superuser with username '$SUPER_USER' already exists. Skipping creation."
else
    /opt/bin/python /BillingSystem/manage.py createsuperuser --username $SUPER_USER --email $SUPERUSER_EMAIL --noinput
